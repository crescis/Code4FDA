function[RMSP,RMSC,Y_full,Y_s1,real_func,alpha_s1] = s1fsl(N,I,M_pc,I0)
...
algpara.iter = 10000;
algpara.lambda = 10;
algpara.lambda1 = 0.01; %control lasso term
algpara.lambda2 = 10; %control sum-of-norms regularizer term
...
pc_s1 = reshape(result_s1(1,:),M_pc,I)';
...
%RMSC = (norm(alpha_s1-real_func,'fro')/(size(real_func,1)*size(real_func,2)))^0.5 
temp = [];
for i = 1:3
    RMSC = (norm(alpha_s1-real_func(:,((i-1)*50+1):i*50),'fro')/(size(real_func,1)*size(real_func,2)))^0.5; 
    temp = [temp,RMSC];
end
RMSC = mean(temp)